...
> **🔎 보강 (2026-05-24 동시성 리뷰): `innodb_lock_wait_timeout`은 1초 권고로 하향.**
> 9-3에서 본 것처럼 PG 호출이 트랜잭션 *밖*이라 락 보유 시간이 ~수십ms다. 같은 사용자 동시 요청이어도 실제 대기는 100ms 미만 → **정상 요청은 1초 타임아웃에 걸리지 않는다.** 1초가 의미 있는 건 비정상 버스트(9-7) 때로, 대기자가 커넥션을 3초가 아니라 1초만 쥐고 반납해 **커넥션 풀을 3배 빨리 회복**한다(9-8). 부작용인 409 증가는 부하를 떨궈야 할 상황에서만 발생하므로 load shedding 관점에서 오히려 바람직. 500ms도 가능하나 GC 멈춤·순간 DB hiccup의 spurious 409를 피해 **1초가 안전한 균형**.
> 추가로 HikariCP `connectionTimeout`(풀에서 커넥션을 받기까지 대기)은 **2~3초** 권고 — 기본 30초는 풀 고갈 시 스레드가 무한정 쌓이게 한다.
...
### 9-8. 커넥션 풀 사이징 (2026-05-24 동시성 리뷰 보강)

9-1 표의 "DB 락 대기 큐 → 커넥션 풀 부담"을 구체화한다.

**실패 모델.** 스택이 Spring MVC + JPA + HikariCP라 **thread-per-request**다. `SELECT ... FOR UPDATE`로 **락을 기다리는 스레드는 그 SELECT를 실행 중인 커넥션을 쥔 채 대기**한다 → **락 경합 = 커넥션 점유**. 핫 row에 대기자가 쌓이면 풀(기본 10)이 고갈되고, 그러면 *그 핫 유저와 무관한 요청*까지 커넥션을 못 받아 죽는다(장애 전파). 단, 본 스코프는 락 단위가 user_id라 정상 트래픽에선 발생하지 않고(9-1), 비정상 버스트(9-7) 때만 현실화된다.

**사이징 원칙.**
- 풀은 **요청 동시성이 아니라 DB의 처리 능력(vCPU)에 맞춘다**: `pool ≈ (코어 수 × 2) + 디스크 수`. 4코어 DB → ~10.
- **락 경합을 흡수하려고 풀을 키우지 않는다(안티패턴)** — 풀을 키우면 더 많은 대기자가 DB에 몰려 핫 row 문제가 *악화*된다. 핫 row는 ① 짧은 lock wait timeout(9-4, 1초) ② 결국 9-7의 분산 락(대기를 커넥션 밖에서)으로 푼다.
- DB가 받는 **총 커넥션 = 풀 × 앱 인스턴스 수 < `max_connections`**(MySQL 기본 151) + 백업/모니터링 여유분. 풀 10이면 인스턴스 10대까지도 100 < 151로 안전.
- 컨테이너냐 EC2냐는 무관 — vCPU 수와 `max_connections`만이 변수다.

**TPS 100에서 10으로 충분한 근거 (Little's Law).**

```
L(동시 사용 커넥션) = λ(처리량) × W(커넥션 점유)
                    = 100 TPS × ~0.05s (트랜잭션1: FOR UPDATE 2 + INSERT/UPDATE 몇, PG는 밖)
                    ≈ 5
```
평균 ~5개 사용 → 풀 10이면 2배 헤드룸. 피크·변동 흡수에 충분.

**권장값 (본 스코프, TPS 100 / 단일 앱 / 단일 DB):**

| 항목 | 권장값 | 비고 |
|---|---|---|
| HikariCP `maximumPoolSize` | **10** (DB 코어에 맞춰 10~20) | TPS 100엔 충분, 키우지 말 것 |
| HikariCP `connectionTimeout` | **2~3초** | 기본 30초는 풀 고갈 시 스레드 누적 |
| `innodb_lock_wait_timeout` | **1초** | 9-4 보강 |

TPS 1000+ 시점엔 read replica로 조회를 분산(차감만 master)한 뒤 master 기준으로 재산정 — 11-2 연계. 단일 풀을 무작정 키우는 게 아니라 아키텍처가 바뀐다.
