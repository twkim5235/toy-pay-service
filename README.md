...
**응답 처리**: 타임아웃 시 즉시 500 반환하지만, "환불 실패 확정"이 아니라 UNKNOWN 상태. 정합성은 보정 배치가 백그라운드에서 회복하며, 클라이언트는 상태 조회 API(`GET /payments/{id}/cancel`)로 최종 결과 확인 가능.
...
#### GET /payments/{paymentId}/cancellation/{idempotencyKey}

특정 멱등성 키 기준 취소 요청 처리 결과 조회. 클라이언트가 POST 응답을 놓친 경우 결과 재조회용.

**Path Parameter**:
| 이름 | 타입 | 설명 |
|---|---|---|
| paymentId | string | 결제 ID |
| idempotencyKey | string | 서버 생성 SHA256 멱등성 키 |

**Response Body**:
POST 응답과 동일 구조. `cancel_idempotency` row에서 status 확인 후, `payment_items` 현재 상태와 함께 응답.

**HTTP 상태 코드**:
| 코드 | 의미 |
|---|---|
| 200 OK | 처리 완료 — 결과 반환 |
| 202 Accepted | 처리 중 (status=PENDING) — 잠시 후 재조회 안내 |
| 404 | 해당 멱등성 키 없음 |
...
| **API** | | |
| Public API | POST 취소 + GET 결과 조회 | RESTful 원칙, 응답 놓친 클라이언트 조회 가능 |
